...
It is built on **Rule-Based Inference**, one of the oldest and most important paradigms in Artificial Intelligence. The same approach was used in **MYCIN (Stanford, 1970s)**, one of the first AI systems designed to assist with diagnosing bacterial infections.

This project demonstrates core AIML concepts including:

* Knowledge Representation using Prolog facts and rules
* Rule-Based Inference using Horn Clauses
...
| Field                   | Details                                    |
| ----------------------- | ------------------------------------------ |
| **Student Name**        | **Anubhav Kushwaha**                       |
| **Registration Number** | **25MIM10001**                             |
...
User answers Yes/No questions
              ↓
Symptoms are stored as Prolog facts:
symptom(fever), symptom(cough), ...
              ↓
Inference Engine (main.pl) calls diagnose/1
              ↓
kb_symptoms.pl is searched
              ↓
A matching rule produces a possible diagnosis
              ↓
kb_treatments.pl provides the recommended action plan
```

The system uses **logical rule-based inference**: it starts from known facts (symptoms) and derives a possible conclusion (diagnosis).
...
RECOMMENDED ACTION:
  ...
  4. Take paracetamol for fever — avoid ibuprofen without
     consulting a doctor.
...
This system is built for **educational purposes only** as part of a university AIML course. It is **not a substitute for professional medical advice, diagnosis, or treatment**.

The diagnoses produced by MediBot are possible rule-based outputs and should not be treated as confirmed medical diagnoses.
...
1. **SWI-Prolog Official Documentation**
   https://www.swi-prolog.org/

2. **MYCIN Expert System** — Shortliffe, E.H. (1976), *Computer-Based Medical*
